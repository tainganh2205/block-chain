...
import { InputArt as Input } from 'components/Art'
...
const TransactionDeadlineSetting = () => {
  ...
  return (
    <StyledTransactionDeadlineSetting>
      <Label>
        <Text className='cl-white' style={{ fontWeight: 'normal' }}>
          <TranslatedText translationId={90}>Transaction deadline</TranslatedText>
        </Text>
        <QuestionHelper text="Your transaction will revert if it is pending for more than this long." />
      </Label>
      <Field>
        <Input  scale="sm" type="number" step="1" min="1" value={value} onChange={handleChange} />
        <Text className='cl-gray'>Minutes</Text>
      </Field>
      {error && (
